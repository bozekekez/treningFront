...
const Tablica = () => {
    ...
    renderTurniri = turniri.map((member) => {
        return(
            <option value={member._id}>{member.turnir}</option>
        )
    })
    ...
    const handleShow = () => {
        fetch('http://localhost:3000/turnir/jedan/?' + new URLSearchParams ({_id: selected}), {
			method: 'get',
			headers: {'Content-Type': 'application/json'},
		}).then(resopnse => resopnse.json())
        .then(temporary => {
...
return(
    <div className="turniriParent">
        <div>
        <select className="turnirSelect" name="turniri" id="turnir" onChange={handleChange} onClick={getTurniri}>
            {renderTurniri}
        </select>
        <button className="turnirTablica" onClick={handleShow}>Prikaži</button>
        </div>
        { render.sudionici ?
